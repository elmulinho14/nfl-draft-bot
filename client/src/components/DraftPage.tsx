...
const DraftPage: React.FC = () => {
  ...
  const [initialPlayerList, setInitialPlayerList] = useState<Player[]>([]); // Store initial list

  // Refs for scrolling
  const activePickRef = useRef<HTMLLIElement | null>(null);
  const submitButtonRef = useRef<HTMLButtonElement | null>(null); // Ref for submit button
  ...
  useEffect(() => {
    ...
    Papa.parse<CsvPlayerRow>('/players.csv', {
      ...
      complete: (results) => {
        ...
        const uniquePositions = ['All', ...Array.from(positionSet)];
        setAllPositions(uniquePositions.sort());

        setAvailablePlayers(playersData);
        setInitialPlayerList(playersData);
        setIsLoading(false);
      },
      error: (error) => {
  ...
  const handleSubmitDraft = async () => {
    if (!draftComplete) {
      console.error('Draft not complete yet!');
      return; // Should not happen if button is only shown when complete
    }
    console.log("Submitting draft:", picks);

    try {
      // Make sure your server URL is correct if running locally vs deployed
      const response = await fetch('/api/drafts', { // Adjust URL if needed
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        // Send the picks data in the correct format expected by the server
      ...
      if (!response.ok) {
        // Handle server errors (e.g., not authenticated, invalid data)
        console.error('Failed to submit draft:', result.error || response.statusText);
        alert(`Error submitting draft: ${result.error || 'Server error'}`);
      } else {
        console.log('Draft submitted successfully:', result);
        alert('Draft submitted successfully!');
        // Reset the draft board on successful submission
        setPicks(round1Teams.map(team => ({ team, player: null })));
        setCurrentPickIndex(0);
        setDraftComplete(false);
        // Reset available players to the full initial list
        setAvailablePlayers(initialPlayerList);
        // Optionally reset filters as well
        setSelectedPositionFilter('All');
        setPlayerNameFilter('');
      }
    } catch (error) {
      console.error('Network or other error submitting draft:', error);
      alert('Failed to submit draft due to a network or client error.');
    }
  };
